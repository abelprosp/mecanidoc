...
A app corre na VPS em `http://72.61.58.208:3000` (pasta `mecanidoc`).
...
sudo ufw allow 3000/tcp   # opcional, acesso direto por IP
sudo ufw reload
```
...
| `http://72.61.58.208:3000` | Continua a funcionar (acesso direto) |

## Ficheiros neste repo

- `deploy/nginx/mecanidoc.com.conf` — config Nginx (proxy → porta 3000)
